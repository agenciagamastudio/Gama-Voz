...
export function Logo({ size = 'md', withText = false, className = '', href = '/landing' }: LogoProps) {
  const sizeClasses = {
    sm: 'w-6 h-6',
    md: 'w-8 h-8',
    lg: 'w-12 h-12',
  ...
  const svg = (
    <svg className={`${sizeClasses[size]} ${className} text-gama-primary inline-block`} viewBox="0 0 545 529" fill="currentColor">
  ...
  if (!withText) {
    return (
      <Link href={href} title="GAMA - Ir para Home" className="hover:opacity-80 transition-opacity inline-block text-gama-primary">
        {svg}
      </Link>
    )
  }
  ...
  return (
    <Link href={href} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
      <div className={`flex-shrink-0 flex items-center justify-center text-gama-primary`}>
        {svg}
      </div>
