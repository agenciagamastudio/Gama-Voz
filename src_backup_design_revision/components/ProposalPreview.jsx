...
function ProposalPreview() {
  ...
  // Redirect if no proposal data is available (e.g., direct access)
  if (!proposalData) {
    navigate('/'); // Or a more appropriate fallback page
    return null;
  }
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen flex flex-col">
      {/* Header Navigation */}
      <header className="sticky top-0 z-10 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 py-3">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold tracking-tight">Pré-visualização da Proposta</h1>
          </div>
          <Link to="/proposal/share" className="px-6 py-2.5 bg-primary text-black font-bold rounded-lg hover:shadow-[0_0_20px_rgba(136,206,17,0.4)] transition-all flex items-center gap-2">
            <span className="material-symbols-outlined text-sm">share</span> {/* Changed from download to share */}
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-200 dark:bg-[#0a0f18] flex flex-col items-center">
        {/* Document Sheet (A4 Proportion Style) */}
        <div className="w-full max-w-[800px] bg-white text-slate-900 rounded-sm document-shadow min-h-[1100px] p-8 md:p-12 mb-10 overflow-hidden">
          {/* Proposal Header */}
