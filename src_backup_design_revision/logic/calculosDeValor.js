...
export const calculateScenarioLoss = (custoHoraEnvolvido, tempoPerdidoOcorrencia, frequencia, numeroPessoasOcorrencias) => {
    ...
    switch (frequencia) {
        case 'dia':
            ...
            perdaSemanal = perdaDiaria * 5; // Assumindo 5 dias úteis por semana
            perdaMensal = perdaDiaria * 22; // Assumindo 22 dias úteis por mês
            break;
        case 'semana':
            perdaSemanal = perdaPorOcorrencia * numeroPessoasOcorrencias;
            perdaMensal = perdaSemanal * (22 / 5); // Proporção de semanas para mês
            break;
        case 'mes':
            perdaMensal = perdaPorOcorrencia * numeroPessoasOcorrencias;
            perdaSemanal = perdaMensal / (22 / 5); // Proporção de mês para semana
            break;
        default:
            break;
    }

    const anual = perdaMensal * 12;
